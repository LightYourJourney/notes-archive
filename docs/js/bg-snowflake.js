...
function gen_particle() {
  ...
  return {
    ...
    opacity: Math.random(),
    sine_components: [
      sine_component(height, 3),
      sine_component(height / 2, 2),
      sine_component(height / 5, 1),
      sine_component(height / 10, 0.5),
    ],
  };
}
...
function update_pos(dt) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    particles[i].x +=
      ((calc_sine(particles[i].sine_components, particles[i].y) * v) / 5) * dt;
    particles[i].y += v * dt;
...
function get_context() {
  ...
  canvas.height = height;
  canvas.style =
    "position: fixed; top: 0; left: 0; overflow: hidden; pointer-events: none; z-index: 256;";
  if (
    (document.documentElement.dataset.darkreaderMode || "").startsWith("filter")
  )
